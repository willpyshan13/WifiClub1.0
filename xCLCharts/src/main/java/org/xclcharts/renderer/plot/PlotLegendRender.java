...
public class PlotLegendRender extends PlotLegend {
    ...
    private void calcContentRect() {
        ...
        float areaWidth = mPlotArea.getWidth() + mXChart.getPaddingLeft() + mXChart.getPaddingRight() - 2 * mMargin;
...
//        drawBox(canvas);
        ...
        while (iter.hasNext()) {
            ...
            if (!"".equals(label)) {
                canvas.drawText(label, currRowX, currRowY + rowHeight - 4, this.getPaint());
            }
