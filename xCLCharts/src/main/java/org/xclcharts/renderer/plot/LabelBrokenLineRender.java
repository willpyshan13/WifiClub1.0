...
public class LabelBrokenLineRender extends LabelBrokenLine {
    ...
    public PointF renderLabelLine(String text, float itemAngle,
        ...
        float brokenLine = getBrokenLine(); //折线长度
        ...
        } else if (Float.compare(stopX + brokenLine, cirX) == 1) //右边
        {
            paintLabel.setTextAlign(Align.LEFT);
            endX = stopX + brokenLine;
            endLabelX = endX + pointRadius;
        } else if (Float.compare(stopX - brokenLine, cirX) == -1) //左边
        {
            paintLabel.setTextAlign(Align.RIGHT);
            endX = stopX - brokenLine;
            endLabelX = endX - pointRadius;
        ...
        if (showLabel) //标签
        {
            if (null == plotLabel) {
                if (text != null && !text.contains("\r\n")) {
                    drawHelper.drawRotateText(text, endLabelX, stopY, itemAngle, canvas, paintLabel);
                } else {
                    assert text != null;
                    String[] split = text.split("\r\n");
                    float height = drawHelper.getPaintFontHeight(paintLabel);
                    drawHelper.drawRotateText(split[0], endLabelX, stopY - height / 2, itemAngle, canvas, paintLabel);
                    drawHelper.drawRotateText(split[1], endLabelX, stopY + height / 2, itemAngle, canvas, paintLabel);
                }
            } else {
