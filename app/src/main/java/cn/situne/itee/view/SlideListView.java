...
import cn.situne.itee.adapter.LocationChoiceDetailsAdapter;
...
public class SlideListView extends ListView {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!canMove
                    ...
                    if (itemView != null) {
                        LocationChoiceDetailsAdapter.ViewHolder myHolder = (LocationChoiceDetailsAdapter.ViewHolder) itemView.getTag();
                        if (offsetX > 0 && (this.mode == MOD_BOTH || this.mode == MOD_RIGHT) && !myHolder.getCanSlide().equals("can")) {
                            /*从右向左滑*/
                            canMove = true;
                        } else /*从左向右滑*/ {
                            canMove = offsetX < 0 && (this.mode == MOD_BOTH || this.mode == MOD_LEFT) && !myHolder.getCanSlide().equals("can");
                        }
                    }
    ...
    private void scrollByDistanceX() {
        ...
        if (itemView.getScrollX() > 0 && (this.mode == MOD_BOTH || this.mode == MOD_RIGHT)) {
            ...
            if (itemView.getScrollX() >= rightLength / 2) {
                if (slideListViewListener != null)
                    slideListViewListener.scrollItem(itemView);
                scrollLeft();
            } else {
                // 滚回到原始位置
                scrollBack();
    ...
    private SlideListViewListener slideListViewListener;

    public SlideListViewListener getSlideListViewListener() {
        return slideListViewListener;
    }

    public void setSlideListViewListener(SlideListViewListener slideListViewListener) {
        this.slideListViewListener = slideListViewListener;
    }

    public interface SlideListViewListener {
