...
/**
 * ClassName:SunTimes <br/>
 * Function: get sunrise and sunset time by longitude and latitude. <br/>
 * Date: 2015-03-11 <br/>
 *
 * @author liaojian
 * @version 1.0
...
public class SunTimes {
    ...
    /*public static void main(String[] args) throws ParseException {
        ...
        System.out.println(GetSunTime(currentTime,116.587099,35.414921 ));
    }*/

    public static String GetSunTime(long day, Double longitude, Double latitude, int timeZone) throws ParseException {
        SunRiset(day, longitude, latitude, -35.0 / 60.0, 1, start, end);
        String sunrise = ToLocalTime(start, timeZone);
        String sunset = ToLocalTime(end, timeZone);
        return sunrise + "&" + sunset;
    }


    private static String ToLocalTime(Double utTime, int timeZone) {
        int hour = (int) (Math.floor(utTime));
        Double temp = utTime - hour;
        hour += timeZone;
        temp = temp * 60;
        int minute = (int) (Math.floor(temp));
        String minuteStr = minute + StringUtils.EMPTY;
        String hourStr = hour + StringUtils.EMPTY;
        if (hour > 24) {
            hour -= 24;
        }
        if (hour < 10) {
        ...
        Utils.log("Sun time : " + hourStr + ":" + minuteStr);
