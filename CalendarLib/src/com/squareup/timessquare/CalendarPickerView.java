...
public class CalendarPickerView extends ListView {
    ...
    private final List<JsonTeeTimeCalendar.DataList.DateStatus> yearDateStatus = new ArrayList<JsonTeeTimeCalendar.DataList.DateStatus>();
    private final List<List<List<MonthCellDescriptor>>> colors =
            new ArrayList<List<List<MonthCellDescriptor>>>();
...
//            tv.setSelected(!tv.isSelected());
     ...
     */
    public FluentInitializer init(Date minDate, Date maxDate, Locale locale, List<JsonTeeTimeCalendar.DataList.DateStatus> dateStatuses, int firstDayOfWeek) {
     ...
     * @param maxDate Latest selectable date, exclusive.  Must be later than {@code minDate}.
     */
    public FluentInitializer init(Date minDate, Date maxDate, List<JsonTeeTimeCalendar.DataList.DateStatus> dateStatus) {
    ...
    private void clearSelectionsWithMonth(int month) {
        ...
        for (int i = 0; i < selectedCells.size(); i++) {
            MonthCellDescriptor selectedCell = selectedCells.get(i);
    ...
    List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal, List<JsonTeeTimeCalendar.DataList.DateStatus> dateStatuses, int firstDay) {
        int flag = 0;
        Calendar cal = Calendar.getInstance(locale);
        cal.setTime(startCal.getTime());


        List<JsonTeeTimeCalendar.DataList.DateStatus.StatusList> statusList = null;
        if (dateStatuses.size() > 0) {
            JsonTeeTimeCalendar.DataList.DateStatus dateStatus = dateStatuses.get(month.getMonth());
            statusList = dateStatus.getStatusList();
        }
        ...
        while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
            ...
            for (int c = 0; c < 7; c++) {
                ...
                if (isCurrentMonth) {
                    if (statusList != null) {
                        JsonTeeTimeCalendar.DataList.DateStatus.StatusList teeTimeStatusList = statusList.get(flag);
                ...
                weekCells.add(new MonthCellDescriptor(date, value, isCurrentMonth, isToday, isSelectable, isSelected, isHighlighted, rangeState, isBookingHere, isNineHoles, isThreeTeeStart, isBlockTimes, isTwoTeeStart, isMemberOnly, isPrimeTime, teeTimeColor));
                cal.add(DATE, 1);
    ...
    private class MonthAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            final MonthDescriptor monthDescriptor = months.get(position);
            if (monthView == null) {
                monthView =
...
//                    v.setSelected(!v.isSelected());
            ...
            monthView.init(monthClickListener, monthDescriptor, cells.get(position), displayOnly, titleTypeface,
