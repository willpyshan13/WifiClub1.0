...
/**
 * ViewGroup that draws a grid of calendar cells.  All children must be {@link CustomCalendarRowView}s.
 * The first row is assumed to be a header and no divider is drawn above it.
 */
public class ChooseCalendarGridView extends ViewGroup {
    /**
    ...
    public ChooseCalendarGridView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setDayBackground(int resId) {
        for (int i = 1; i < getChildCount(); i++) {
            ((ChooseCalendarRowView) getChildAt(i)).setCellBackground(resId);
        }
    }
    ...
    public void setDayTextColor(int resId) {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            ((ChooseCalendarRowView) getChildAt(i)).setCellTextColor(colors);
        }
    }
    ...
    public void setHeaderTextColor(int color) {
        ((ChooseCalendarRowView) getChildAt(0)).setCellTextColor(color);
    }

    public void setTypeface(Typeface typeface) {
        for (int i = 0; i < getChildCount(); i++) {
            ((ChooseCalendarRowView) getChildAt(i)).setTypeface(typeface);
        }
    }
    ...
    public void addView(View child, int index, LayoutParams params) {
        if (getChildCount() == 0) {
            ((ChooseCalendarRowView) child).setIsHeaderRow(true);
        }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        for (int c = 0; c < 7; c++) {
            ...
            canvas.drawLine(x, top, x, bottom * 2, dividerPaint);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        final int rowHeightSpec = makeMeasureSpec(cellSize * 2, EXACTLY);
        ...
        final int measuredWidth = widthMeasureSize + 10; // Fudge factor to make the borders show up.
